...
if (wrapper.exists()) {
  ...
  if(System.getenv("DEBUG") != "false) println("Running [$wrapper, $arguments] from ${wrapper.parentFile}")#!/usr/bin/env -S ktx execute
/**
 * A convenience wrapper script that finds the nearest `gradlew` script
 * and appends gradle project path relative to it to all task arguments.</p>
 *
...
if (wrapper.exists()) {
  ...
  println("Running [$wrapper, $arguments] from ${wrapper.parentFile}")
